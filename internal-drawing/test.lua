...
Drawing = Drawing or require(script.Parent.Drawing)
...
library.new = function(esp_type : string, player : Player, ... : {})
	...
	if esp_type == 'Highlight' then
		...
		local instance = Instance.new('Highlight', player:WaitForChild('PlayerGui'))
	...
	if esp_type == 'Skeleton' then
		...
		local lines = {
			...
			--> Left leg
			['LeftFoot'] = newLine(),
			['LeftLowerLeg'] = newLine(),
			['LeftUpperLeg'] = newLine(),

			--> Right leg
			['RightFoot'] = newLine(),
			['RightLowerLeg'] = newLine(),
			['RightUpperLeg'] = newLine(),
		}  
			...
			if char then
				local s, e = pcall(function()
					...
					attachments = {
						...
						LeftLowerLeg = char["LeftLowerLeg"]["LeftKneeRigAttachment"],
						LeftUpperLeg = char["LowerTorso"]["LeftHipRigAttachment"],

						--> Right Leg <--------------------------------------------------------
						RightFoot = char["RightFoot"]["RightFootAttachment"],
						RightLowerLeg = char["RightLowerLeg"]["RightKneeRigAttachment"],
						RightUpperLeg = char["RightUpperLeg"]["RightHipRigAttachment"],
					}

					if on_screen and data.Enabled then
						--> Head <-------------------------------------------------------------
						local HeadVector, _ = camera:WorldToViewportPoint(char.Head.Position - Vector3.new(0, 0.07, 0))
						...
						lines.LeftElbow.From = getWorldVector2(attachments.LeftShoulder)
						lines.LeftElbow.To = getWorldVector2(attachments.Neck)
						...
						lines.RightElbow.From = getWorldVector2(attachments.RightShoulder)
						lines.RightElbow.To = getWorldVector2(attachments.Neck)

						--> Left Leg <---------------------------------------------------------
						lines.LeftFoot.From = getWorldVector2(attachments.LeftFoot)
						lines.LeftFoot.To = getWorldVector2(attachments.LeftLowerLeg)
						lines.LeftLowerLeg.From = getWorldVector2(attachments.LeftLowerLeg)
						lines.LeftLowerLeg.To = getWorldVector2(attachments.LeftUpperLeg)
						lines.LeftUpperLeg.From = getWorldVector2(attachments.LeftUpperLeg)
						lines.LeftUpperLeg.To = getWorldVector2(attachments.Waist)

						--> Right Leg <--------------------------------------------------------
						lines.RightFoot.From = getWorldVector2(attachments.RightFoot)
						lines.RightFoot.To = getWorldVector2(attachments.RightLowerLeg)
						lines.RightLowerLeg.From = getWorldVector2(attachments.RightLowerLeg)
						lines.RightLowerLeg.To = getWorldVector2(attachments.RightUpperLeg)
						lines.RightUpperLeg.From = getWorldVector2(attachments.RightUpperLeg)
						lines.RightUpperLeg.To = getWorldVector2(attachments.Waist)
		...
		local function RayCheck(object)
			local Camera = workspace.CurrentCamera
			...
			Parameters.FilterDescendantsInstances = { object.Parent }
			
			local Ray = workspace:Raycast(Origin, Direction, Parameters)
		...
		task.spawn(function()
			while task.wait() do
				for i,v in attachments do
					local Camera = workspace.CurrentCamera
					local _, VisCheck = Camera:WorldToViewportPoint(v.WorldPosition)
					...
					if RayCheck(v.Parent) then
						--> print(i .. ' is blocked by something')
						local s,e = pcall(function()lines[i].Color = _G.WallCheckColors[1] end)
					elseif not RayCheck(v.Parent) then
						--> print(i .. ' is not blocked by something')
						local s,e = pcall(function()lines[i].Color = _G.WallCheckColors[2] end)
					end
					--if VisCheck then
					--	pcall(function()
					--		--print(i)
